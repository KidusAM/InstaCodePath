...
//        queryPosts()
        ...
        findViewById<Button>(R.id.btnSubmitPost).setOnClickListener {
            val description = findViewById<EditText>(R.id.etPostDescription).text.toString()
            val user = ParseUser.getCurrentUser()
            if (photoFile != null) {
                submitPost(description, user, photoFile!!)
            } else {
                Log.e(TAG, "Photo is null on the post")
            }
        }
    ...
    private fun submitPost(description : String, user: ParseUser, photoFile : File) {
        ...
        post.saveInBackground { e ->
            if (e != null) {
                ...
                e.printStackTrace()
                Toast.makeText(this, "Error submitting post to server", Toast.LENGTH_SHORT).show()
            } else {
                Log.i(TAG, "successfully saved the post")
            }
        }
    }
    // query for all the posts on the server
